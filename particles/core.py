...
class Particle:
    ...
    def __init__(self, id, pos_x=0.0, pos_y=0.0, velocity_x=0.0,
                 velocity_y=0.0):
        if isinstance(id, bytes):
            (self.pos_x, self.pos_y, self.velocity_x, self.velocity_y,
             self.id) = struct.unpack(self.STRUCT_FORMAT, id)
        else:
            self.pos_x = pos_x
            self.pos_y = pos_y
    ...
    def __str__(self):
        return "Particle ({id}; {pos_x}; {pos_y}; {v_x}; {v_y})".format(
            pos_x=self.pos_x,
            pos_y=self.pos_y,
            v_x=self.velocity_x,
            v_y=self.velocity_y,
    ...
    def __bytes__(self):
        return struct.pack(self.STRUCT_FORMAT, self.pos_x, self.pos_y,
                           self.velocity_x, self.velocity_y, self.id)
    ...
    def is_approaching(self, other_particle):
        """Return true if particles are approaching each other by at least one
    ...
    def distance_to(self, other_particle):
        """Calculate the distance between the current and the specified
        particle centers.
    ...
    def overlaps(self, other, particle_r):
        """
        Check whether the particle overlaps another particle, assuming equal
         radius.
