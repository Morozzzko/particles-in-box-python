...
class TestParticleBehavior(unittest.TestCase):
    def setUp(self):
        self.particle = Particle(id=1, pos_x=13.4, pos_y=12.3, velocity_x=3.0,
                                 velocity_y=3.4)

    def tearDown(self):
        pass

    def copy_particle(self, offset_x=0.0, offset_y=0.0, v_ratio_x=1.0,
                      v_ratio_y=1.0):
        return Particle(
            id=self.particle.id + 3,
            # invert the lesser bit, also increase the index
    ...
    def test_particles_overlap_symmetric(self):
        particle_r = 1.0
        particle_b = self.copy_particle(offset_x=particle_r / 10,
                                        offset_y=particle_r / 5)
    ...
    def test_particle_speed(self):
        self.assertAlmostEqual(self.particle.speed(),
                               sqrt(
                                   self.particle.velocity_x ** 2 +
                                   self.particle.velocity_y ** 2))
    ...
    def test_particle_approaching_symmetric(self):
        offset = 2.0
        particle_b = self.copy_particle(offset_x=offset, offset_y=offset,
                                        v_ratio_x=-1.0, v_ratio_y=-1.0)
        self.assertEqual(self.particle.is_approaching(particle_b),
                         particle_b.is_approaching(self.particle))
