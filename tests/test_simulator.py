...
class TestNewSimulation(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.particle_arrays = [[1.11, 2.22, 3.33, 4.44, 1],
                                ...
                                [3, 6.66, 7.77, 8.88, 2]]
        file.write(pack(BinarySimulator.str_decode, *self.parameters))
    ...
    def test_binary_simulator_init(self):
        simulator = BinarySimulator(self.path)
        params = tuple(self.parameters[:11] + [self.parameters[11] + self.parameters[12], self.parameters[14]])
        self.assertEqual(params, self.get_simulator_parameters(simulator))

    def test_binary_simulator_next_state(self):
        simulator = BinarySimulator(self.path)
        i = len(self.particle_arrays) - len(simulator.particles)
